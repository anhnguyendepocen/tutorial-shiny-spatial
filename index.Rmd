...
These data are in the package `SpatialEpiApp` (https://paula-moraga.github.io/software/) and can also be downloaded from XX.
...
Shiny is a web application framework for R that enables to build interactive web applications (https://shiny.rstudio.com/).
...
We download the folder `appdir` from https://paula-moraga.github.io/software/ and save it in our computer. This folder contains the following subfolders:
...
These are HTML widgets created with JavaScript libraries and embedded in Shiny by using the `htmlwidgets` package (https://www.htmlwidgets.org/).
...
We create the leaflet map following the instructions in
...
Note that a better way to modify an existing leaflet map is using the `leafletProxy()` function. Details on how to use this function are given in https://rstudio.github.io/leaflet/shiny.html.
...
Details on how to use `req()` are in https://shiny.rstudio.com/articles/req.html.
